...
describe("vault_init.ts", () => {
  ...
  it("errors when vault PDA already exists and is owned by program", async () => {
    jest.resetModules();

    const initializeVaultMock = jest.fn();
    const methods = { initializeVault: initializeVaultMock };
    ...
    jest.doMock("@solana/web3.js", () => {
      ...
      const programId = new (PublicKey as any)("6YwvmcWvd2ijBN3ecMhi3VJ2ghmgRbvuWMboHk3JSkPu");

      return {
        Commitment: {} as any,
        Connection: function () {
          return {
            getAccountInfo: jest.fn(async () => ({ owner: programId })),
          };
        },
        Keypair: {
          fromSecretKey: (_u8: Uint8Array) => ({ publicKey: new (PublicKey as any)("SENDER") }),
    ...
    const argvBak = process.argv;
    process.argv = ["node", "vault_init.ts"]; // defaults
    ...
    expect(initializeVaultMock.mock.calls.length).to.eq(0);
    expect(String(errSpy.mock.calls[0][0])).to.contain("Vault PDA already initialized");

    errSpy.mockRestore();
    process.argv = argvBak;
  });

  it("errors when vault PDA already exists but is owned by a different program", async () => {
    jest.resetModules();
    ...
    expect(initializeVaultMock.mock.calls.length).to.eq(0);
    expect(String(errSpy.mock.calls[0][0])).to.contain("Vault PDA address is already in use");
